package services.dao.Fax2EMR.model;


public class Fax2EMRLaboratoryReport {
	Integer id;
	String Patient_name;
	String Phone_number;
	String Patient_ID;
	String Physician;
	String Sex;
	String Age;
	String Date_of_birth;
	String Sodium;
	String Potassium;
	String Chloride;
	String Carbon_dioxide;
	String Calcium;
	String Alkaline_phosphatase;
	String AST;
	String ALT;
	String Bilirubin_total;
	String Glucose;
	public Integer getId() {
		return this.id;
	}
	public String getPatientName() {
		return this.Patient_name;
	}
	public String getPhoneNumber() {
		return this.Phone_number;
	}
	public String getPatientID() {
		return this.Patient_ID;
	}
	public String getPhysician() {
		return this.Physician;
	}
	public String getSex() {
		return this.Sex;
	}
	public String getAge() {
		return this.Age;
	}
	public String getDateOfBirth() {
		return this.Date_of_birth;
	}
	public String getSodium() {
		return this.Sodium;
	}
	public String getPotassium() {
		return this.Potassium;
	}
	public String getChloride() {
		return this.Chloride;
	}
	public String getCarbonDioxide() {
		return this.Carbon_dioxide;
	}
	public String getCalcium() {
		return this.Calcium;
	}
	public String getAlkalinePhosphatase() {
		return this.Alkaline_phosphatase;
	}
	public String getAST() {
		return this.AST;
	}
	public String getALT() {
		return this.ALT;
	}
	public String getBilirubinTotal() {
		return this.Bilirubin_total;
	}
	public String getGlucose() {
		return this.Glucose;
	}
	public void setId(Integer id) {
		this.id = id;
	}
	public void setPatientName(String Patient_name) {
		this.Patient_name = Patient_name;
	}
	public void setPhoneNumber(String Phone_number) {
		this.Phone_number = Phone_number;
	}
	public void setPatientID(String Patient_ID) {
		this.Patient_ID = Patient_ID;
	}
	public void setPhysician(String Physician) {
		this.Physician = Physician;
	}
	public void setSex(String Sex) {
		this.Sex = Sex;
	}
	public void setAge(String Age) {
		this.Age = Age;
	}
	public void setDateOfBirth(String Date_of_birth) {
		this.Date_of_birth = Date_of_birth;
	}
	public void setSodium(String Sodium) {
		this.Sodium = Sodium;
	}
	public void setPotassium(String Potassium) {
		this.Potassium = Potassium;
	}
	public void setChloride(String Chloride) {
		this.Chloride = Chloride;
	}
	public void setCarbonDioxide(String Carbon_dioxide) {
		this.Carbon_dioxide = Carbon_dioxide;
	}
	public void setCalcium(String Calcium) {
		this.Calcium = Calcium;
	}
	public void setAlkalinePhosphatase(String Alkaline_phosphatase) {
		this.Alkaline_phosphatase = Alkaline_phosphatase;
	}
	public void setAST(String AST) {
		this.AST = AST;
	}
	public void setALT(String ALT) {
		this.ALT = ALT;
	}
	public void setBilirubinTotal(String Bilirubin_total) {
		this.Bilirubin_total = Bilirubin_total;
	}
	public void setGlucose(String Glucose) {
		this.Glucose = Glucose;
	}
}
